...
Deterministic Task Planner v7.0
...
class DynamicBrainScorer:
    """
    LEVI v6: Adaptive Decision Logic. 
    Routes queries based on dynamic weights that evolve as the system learns.
    """
    DEFAULT_KEYWORDS = {
    ...
    async def get_routing_config() -> Dict[str, Any]:
        """Loads routing weights from Redis/Firestore."""
        from backend.db.redis_client import HAS_REDIS, get_cached_json, cache_json
        from backend.db.firestore_db import db as firestore_db
        
        cache_key = "system:routing_config"
        if HAS_REDIS:
            cached = get_cached_json(cache_key)
            if cached: return cached

        try:
            doc = await asyncio.to_thread(firestore_db.collection("system").document("routing_config").get)
            config = doc.to_dict() if doc.exists else {"keywords": DynamicBrainScorer.DEFAULT_KEYWORDS, "boosts": {}}
            if HAS_REDIS: cache_json(cache_key, config, ttl=3600)
            return config
        except Exception:
            return {"keywords": DynamicBrainScorer.DEFAULT_KEYWORDS, "boosts": {}}

    @staticmethod
    async def calculate_scores(user_input: str) -> Dict[str, float]:
        text = user_input.lower()
        config = await DynamicBrainScorer.get_routing_config()
        keywords = config.get("keywords", DynamicBrainScorer.DEFAULT_KEYWORDS)
        boosts = config.get("boosts", {})

        scores = {"chat": 0.1, "search": 0.0, "document": 0.0}

        # 1. Keyword Presence (Dynamic Weights)
        for route, kws in keywords.items():
            route_boost = boosts.get(route, 1.0)
            for kw in kws:
                if kw in text:
                    scores[route] += 0.3 * route_boost
        
        # 2. Pattern Matching
        if re.search(r"\b(pdf|docx|txt|file|paper)\b", text): scores["document"] += 0.4 * boosts.get("document", 1.0)
        if re.search(r"\b(current|latest|news|live|real-time)\b", text): scores["search"] += 0.4 * boosts.get("search", 1.0)
        if re.search(r"^\s*(hi|hello|hey|yo|howdy)\s*", text): scores["chat"] += 0.5 * boosts.get("chat", 1.0)

        return scores

async def detect_orchestration_route(user_input: str) -> Dict[str, Any]:
    """
    Determines the target engine route (chat | search | document).
    """
    scores = await DynamicBrainScorer.calculate_scores(user_input)
...
async def check_rules(user_input: str) -> Optional[IntentResult]:
    ...
    orch = await detect_orchestration_route(user_input)
    return IntentResult(
...
def detect_sensitivity(user_input: str) -> bool:
    ...
    Scans for: Emails, Credit Cards, SSNs, and explicit 'Bank/Secret/Private' keywords.
    """
    text = user_input.lower()
...
async def detect_intent(user_input: str) -> IntentResult:
    # 0. Sovereign Shield Check
    is_sensitive = detect_sensitivity(user_input)
    
    # 1. High-Speed Rule-Based Detection
    rule_match = await check_rules(user_input)
    if rule_match:
        rule_match.is_sensitive = is_sensitive
        return rule_match

    # If sensitive, we force a conservative local path
    if is_sensitive:
        return IntentResult(
            intent_type="sensitive_local",
            complexity_level=1,
            confidence_score=1.0,
            is_sensitive=True,
            parameters={"routing": {"reason": "Sovereign Shield Triggered"}}
        )

    # 1. Collective Wisdom Injection (Phase 5)
    from .memory_utils import retrieve_resonant_patterns
    collective_wisdom = await retrieve_resonant_patterns(intent.parameters.get("topic") or user_input)
    wisdom_context = ""
    if collective_wisdom:
        wisdom_context = "\n### COLLECTIVE WISDOM (RELEVANT SUCCESS PATTERNS):\n"
        for i, p in enumerate(collective_wisdom):
            wisdom_context += f"Pattern {i+1}:\nInput: {p['input']}\nStrategy: {p['output']}\n\n"
        logger.info(f"[Planner] Injected {len(collective_wisdom)} resonant patterns from hive.")

    # 2. Planning Logic Execution
    system_prompt = (
        "You are the LEVI Master Planner. Decompose this request into a high-fidelity Execution Plan.\n\n"
        f"{wisdom_context}"
        "AVAILABLE AGENTS:\n"
...
async def generate_plan(user_input: str, intent_data: IntentResult, context: Dict[str, Any]) -> ExecutionPlan:
    ...
    return ExecutionPlan(
        ...
        complexity_level=intent_data.complexity_level,
        is_sensitive=intent_data.is_sensitive
    )
