...
from backend.db.redis_client import cache_search, get_cached_search, HAS_REDIS  # type: ignore
from backend.db.vector_store import embed_text  # type: ignore
# sd_gen_image replaced by local imports below if needed
...
from backend.db.gcs_utils import upload_image_to_gcs # type: ignore
...
def generate_quote_image(
    ...
    if bg is None:
        try:
            from backend.services.image_gen import generate as sd_generate  # type: ignore
    ...
    if upload_to_storage and (os.getenv("AWS_S3_BUCKET") or GCP_STORAGE_BUCKET):
        ...
        from backend.services.studio.utils import upload_image_to_s3
