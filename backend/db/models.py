...
class Artifact(Base):
    """
    Sovereign v16.1: Mission Artifact Tracker.
    Tracks files generated by the cognitive swarm for P2P chunked transfer.
    """
    ...
    file_path = Column(String, nullable=False) # Absolute path on local node
    file_type = Column(String) # e.g., 'image/png', 'text/log'
    file_size = Column(Integer)
    checksum = Column(String) # SHA-256 for integrity
    
    node_id = Column(String, index=True) # Origin node
