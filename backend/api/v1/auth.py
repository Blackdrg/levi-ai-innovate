...
from backend.auth.logic import get_current_user, get_current_user_optional
...
async def login(response: Response, payload: dict):
    ...
    """
    from backend.auth.logic import get_current_user # ensure we can use it
    uid = payload.get("uid")
...
async def logout(current_user: dict = Depends(get_current_user)):
    ...
    """
    from backend.db.redis import HAS_REDIS, r as redis
    if not HAS_REDIS:
