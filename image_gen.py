...
def generate_quote_image(
    ...
    custom_bg:  str   = None,
    user_tier:  str   = "free",
    return_pil: bool  = False,          # ← NEW: return PIL Image instead of BytesIO
) -> "BytesIO | Image.Image":
    """
    Generate a quote image.

    Args:
        return_pil: If True, returns a PIL Image (needed by video_gen.py).
                    If False (default), returns a BytesIO PNG stream.
    """
    ...
    # --- Step 5: Return ---
    if return_pil:
        return bg.convert("RGB")          # PIL Image for video_gen.py

    output = BytesIO()
    bg.convert("RGB").save(output, "PNG", optimize=True)
    output.seek(0)
    return output                         # BytesIO for API responses
