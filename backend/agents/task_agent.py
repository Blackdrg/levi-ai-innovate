...
class TaskAgent(SovereignAgent[TaskInput, AgentResult]):
    ...
    async def _run(self, input_data: TaskInput, lang: str = "en", **kwargs) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "- Delegation: Optimize for agent specialization (Artisan, Librarian, ResearchArchitect, ImageArchitect, VideoArchitect, Critic, etc.).\n"
            "Output ONLY valid JSON:\n"
            "{\n"
            "  \"strategy\": \"High-level approach summary\",\n"
            "  \"steps\": [\n"
            "    {\"step\": 1, \"task\": \"Task description\", \"agent\": \"Artisan\", \"reason\": \"Execution required\"}\n"
            "  ]\n"
            "}"
        )
