...
from backend.db.redis_client import HAS_REDIS, REDIS_URL, r as redis_client
from backend.api import chat, studio, documents, auth, payments, learning, gallery
from backend.services.auth.logic import get_current_user, get_current_user_optional, verify_admin, verify_internal_service, verify_system_admin
from backend.config.system import ENVIRONMENT, SECRET_KEY, CORS_ORIGINS
...
async def finetune_poller():
    ...
    from backend.core.fine_tune_tasks import poll_finetune_status
    while True:
        try:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from backend.services.gateway.broadcast import register_broadcaster
...
async def get_daily_quote(mood: str = "philosophical"):
    ...
    from backend.engines.chat.generation import fetch_open_source_quote
...
async def activity_stream(request: Request):
    ...
    async def event_generator():
        from backend.db.redis_client import get_async_redis
