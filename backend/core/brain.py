...
from backend.services.learning.logic import AdaptivePromptManager, collect_training_sample
...
class LeviBrain:
    ...
    async def route(
        ...
        **kwargs
        ...
        from backend.db.redis_client import check_exact_match, store_exact_match
        ...
        from backend.db.redis_client import check_semantic_match
        ...
        from backend.config.system import COST_MATRIX
        ...
        from backend.services.learning.logic import retrieve_resonant_patterns
        ...
        if decision.complexity_level == 0:
            ...
            from backend.db.redis_client import store_exact_match
    ...
    def _trigger_background_tasks(self, user_input: str, response: str, context: Dict[str, Any]):
        ...
        if user_id and not str(user_id).startswith("guest:"):
            ...
            from backend.db.redis_client import store_exact_match, store_semantic_match
            store_exact_match(user_id, user_input, context.get("mood", "philosophical"), response)
            ...
            for res in results:
                if not res.success:
                    ...
                    from backend.db.redis_client import incr_failure_count
            ...
            if HAS_REDIS:
                ...
                if context.get("rating", 0) >= 5:
                    from backend.services.learning.logic import collect_global_pattern
...
# ── Functional Entry Point (v6 Compatibility) ────────────────
_brain_instance = LeviBrain()

async def run_orchestrator(**kwargs):
    """
    Standard entry point for LEVI-AI orchestration.
    """
    return await _brain_instance.route(**kwargs)
